...
public abstract class ScreenCustomContainer<T extends Container> extends ContainerScreen<T> implements IHasContainer<T>
{
    ...
    protected void func_230450_a_(final PoseStack renderStack, final float partialTicks, final int mouseX, final int mouseY) {
        ...
        RenderingUtils.draw(7, DefaultVertexFormat.field_227851_o_, buf -> RenderingGuiUtils.rect((VertexConsumer)buf, renderStack, (float)this.field_147003_i, (float)this.field_147009_r, (float)this.func_230927_p_(), (float)this.sWidth, (float)this.sHeight).draw());
    }
}
