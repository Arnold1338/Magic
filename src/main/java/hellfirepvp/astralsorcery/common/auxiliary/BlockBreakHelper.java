...
{
    private static final Map<ResourceKey<Level>, TickTokenMap<BlockPos, BreakEntry>> breakMap;
    ...
    public static void addProgress(final Level world, final BlockPos pos, final float percStrength, final Supplier<Float> expectedHardness) {
        final TickTokenMap<BlockPos, BreakEntry> map = BlockBreakHelper.breakMap.computeIfAbsent((ResourceKey<Level>)world.dimension(), key -> {
    ...
    static {
        breakMap = new HashMap<ResourceKey<Level>, TickTokenMap<BlockPos, BreakEntry>>();
    }
    ...
    public static class BreakEntry implements TickTokenMap.TickMapToken<Float>, CEffectAbstractList.ListEntry
    {
        ...
        public void onTimeout() {
            ...
            if (this.world instanceof ServerLevel && BlockUtils.matchStateExact(this.expected, nowAt)) {
                BlockUtils.breakBlockWithoutPlayer((ServerLevel)this.world, this.pos, this.world.getBlockState(this.pos), ItemStack.EMPTY, true, true);
            }
        }
