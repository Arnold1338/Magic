    ...
    public static JournalPageLiquidInfusion fromOutput(final Predicate<ItemStack> outputTest) {
        return new JournalPageLiquidInfusion(() -> {
            ...
            if (mgr == null) {
                ...
                return (LiquidInfusion)mgr.func_215366_a((RecipeType)RecipeTypesAS.TYPE_INFUSION.getType()).values().stream().map(r -> (LiquidInfusion)r).filter(r -> outputTest.test(r.getOutput(ItemStack.field_190927_a))).findFirst().orElse(null);
            }
        });
    }
