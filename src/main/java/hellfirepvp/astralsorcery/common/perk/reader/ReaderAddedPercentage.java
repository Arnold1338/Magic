    ...
    public PerkStatistic getStatistics(final PerkAttributeMap statMap, final Player player) {
        ...
        if (Math.abs(value - postValue) > 1.0E-4 && (limit == null || Math.abs(postValue - limit) > 1.0E-4)) {
            if (Math.abs(postValue) >= 1.0E-4) {
                postProcess = net.minecraft.client.resources.language.I18n.func_135052_a("perk.reader.astralsorcery.postprocess.default", new Object[] { ((postValue >= 0.0) ? "+" : "") + PerkAttributeReader.formatDecimal(postValue) + "%" });
            }
            value = postValue;
        }
