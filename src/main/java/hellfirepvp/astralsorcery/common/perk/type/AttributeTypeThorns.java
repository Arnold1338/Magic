...
public class AttributeTypeThorns extends PerkAttributeType
{
    ...
    private void onThronsReflect(final LivingHurtEvent event) {
        ...
        if (reflectTarget == null && AttributeEvent.postProcessModded(player, this, PerkAttributeHelper.getOrCreateMap(player, side).getModifier(player, prog, PerkAttributeTypesAS.ATTR_TYPE_INC_THORNS_RANGED)) > 1.0f && source.getDirectEntity() != null && source.getDirectEntity() instanceof LivingEntity && reflectTarget.isAlive() {
            reflectTarget = (LivingEntity)source.getDirectEntity();
        }
