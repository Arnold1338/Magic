...
public class KeyVoidTrash extends KeyPerk
{
    ...
    public static class Config extends ConfigEntry
    {
        ...
        public void createEntries(final ForgeConfigSpec.Builder cfgBuilder) {
            this.trashItems = (ForgeConfigSpec.ConfigValue<List<String>>)cfgBuilder.comment("List items that should count as trash and should be voided.").translation(this.translationKey("trashItems")).define("trashItems", KeyVoidTrash.defaultTrashItems.stream().map(item -> net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(item).toString()).collect(Collectors.toList()));
