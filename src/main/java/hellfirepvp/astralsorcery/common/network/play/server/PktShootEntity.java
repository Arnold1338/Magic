        ...
        return new Handler<PktShootEntity>() {
            ...
            public void handleClient(final PktShootEntity packet, final NetworkEvent.Context context) {
                context.enqueueWork(() -> {
                    ...
                    final Entity entity = world.map(w -> w.func_73045_a(packet.entityId)).orElse(null);
                    if (entity != null) {
                        entity.func_213317_d(packet.motionVector.toVector3d());
                        if (packet.hasEffect) {
                            final Vector3 origin = Vector3.atEntityCenter(entity).setY(entity.func_226278_cu_() + entity.func_213302_cg());
