...
public class ContainerAltarDiscovery extends ContainerAltarBase
{
    ...
    void bindPlayerInventory(final Inventory plInventory) {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.render(new Slot((Container)plInventory, j + i * 9 + 9, 8 + j * 18, 84 + i * 18));
            }
        }
        for (int i = 0; i < 9; ++i) {
            this.render(new Slot((Container)plInventory, i, 8 + i * 18, 142));
        }
    }
    ...
    void bindAltarInventory(final TileInventory altarInventory) {
        for (int xx = 0; xx < 3; ++xx) {
            this.render((Slot)new SlotItemHandler((IItemHandler)altarInventory, 6 + xx, 62 + xx * 18, 11));
        }
        for (int xx = 0; xx < 3; ++xx) {
            this.render((Slot)new SlotItemHandler((IItemHandler)altarInventory, 11 + xx, 62 + xx * 18, 29));
        }
        for (int xx = 0; xx < 3; ++xx) {
            this.render((Slot)new SlotItemHandler((IItemHandler)altarInventory, 16 + xx, 62 + xx * 18, 47));
        }
    }
