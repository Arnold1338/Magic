...
public class ContainerAltarDiscoveryProvider extends CustomContainerProvider<ContainerAltarDiscovery>
{
    ...
    public ContainerAltarDiscoveryProvider(final TileAltar ta) {
    ...
    @Nonnull
    @Override
    public ContainerAltarDiscovery createMenu(final int id, final Inventory plInventory, final Player player) {
        return new ContainerAltarDiscovery(this.ta, plInventory, id);
    }
    
    private static ContainerAltarDiscovery createFromPacket(final int id, final Inventory plInventory, final FriendlyByteBuf data) {
        final BlockPos at = ByteBufUtils.readPos(data);
        final Player player = plInventory.field_70458_d;
        final TileAltar ta = MiscUtils.getTileAt((BlockGetter)player.level(), at, TileAltar.class, true);
    ...
    {
        public ContainerAltarDiscovery create(final int windowId, final Inventory inv, final FriendlyByteBuf data) {
