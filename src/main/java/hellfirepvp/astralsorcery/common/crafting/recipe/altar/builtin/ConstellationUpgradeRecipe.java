...
{
    public ConstellationUpgradeRecipe(final ResourceLocation recipeId, final AltarType altarType, final int duration, final int starlightRequirement, final AltarRecipeGrid recipeGrid) {
        super(recipeId, altarType, duration, starlightRequirement, recipeGrid);
        this.addAltarEffect(AltarRecipeEffectsAS.UPGRADE_ALTAR);
    }
    
    public static ConstellationUpgradeRecipe convertToThis(final SimpleAltarRecipe other) {
        return new ConstellationUpgradeRecipe(other.func_199560_c(), other.getAltarType(), other.getDuration(), other.getStarlightRequirement(), other.getInputs());
    }
    ...
    public void onRecipeCompletion(final TileAltar altar, final ActiveSimpleAltarRecipe activeRecipe) {
        ...
        ResearchManager.informCraftedAltar(altar, activeRecipe, new ItemStack((ItemLike)BlocksAS.ALTAR_CONSTELLATION));
        altar.func_145831_w().func_180501_a(altar.func_174877_v(), BlocksAS.ALTAR_CONSTELLATION.defaultBlockState(), 3);
    }
}
