...
{
    public ConstellationBaseMergeStatsRecipe(final ResourceLocation recipeId, final AltarType altarType, final int duration, final int starlightRequirement, final AltarRecipeGrid recipeGrid) {
        super(recipeId, altarType, duration, starlightRequirement, recipeGrid);
    }
    
    public static ConstellationBaseMergeStatsRecipe convertToThis(final SimpleAltarRecipe other) {
    ...
    private void setStats(final ItemStack out, final Iterable<ItemStack> inventoryContents) {
        ...
        final CrystalAttributes attr2 = builder.build();
        if (!attr2.isEmpty()) {
