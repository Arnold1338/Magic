...
public class ActiveSimpleAltarRecipe
{
    ...
    public void consumeInputs(final TileAltar altar) {
        ...
        for (final CraftingFocusStack input2 : this.focusStacks) {
            final TileSpectralRelay tar = MiscUtils.getTileAt((BlockGetter)altar.getLevel(), input2.getRealPosition(), TileSpectralRelay.class, true);
            if (tar != null) {
    ...
    public boolean matches(final TileAltar altar, final boolean ignoreStarlightRequirement, final boolean testNecessaryRelayInputs) {
        ...
        for (final CraftingFocusStack stack : this.focusStacks) {
            ...
            final TileSpectralRelay relay = MiscUtils.getTileAt((BlockGetter)altar.getLevel(), stack.getRealPosition(), TileSpectralRelay.class, true);
            if (relay == null) {
                return false;
            }
        ...
        for (int i = 0; i < iIngredients.size(); ++i) {
            ...
            if (this.ticksCrafting >= offset) {
                ...
                if (found == null) {
                    ...
                    final TileSpectralRelay tar = MiscUtils.getTileAt((BlockGetter)altar.getLevel(), at, TileSpectralRelay.class, true);
                    if (tar == null) {
                        waitMissingInputs = true;
                ...
                final TileSpectralRelay tar2 = MiscUtils.getTileAt((BlockGetter)altar.getLevel(), found.getRealPosition(), TileSpectralRelay.class, true);
                if (tar2 == null) {
                    waitMissingInputs = true;
                }
