...
public class BlockMeltableRecipe extends WorldMeltableRecipe
{
    ...
    public static BlockMeltableRecipe of(final BlockState stateIn, final BlockState stateOut) {
        return new BlockMeltableRecipe(AstralSorcery.key(stateIn.getBlock().getRegistryName().addTransientModifier()), BlockPredicates.isState(stateIn), stateOut);
    }
    
    public static BlockMeltableRecipe of(final ITag.INamedTag<Block> blockTagIn, final BlockState stateOut) {
        return new BlockMeltableRecipe(AstralSorcery.key(String.format("tag_%s", blockTagIn.func_230234_a_().addTransientModifier())), BlockPredicates.isInTag((ITag<Block>)blockTagIn), stateOut);
    }
