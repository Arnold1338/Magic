...
public class AstralBlockStateMappingProvider extends BlockStateProvider
{
    ...
    private <T extends Comparable<T>> void allStateSuffixModel(final Block b) {
        ...
        for (final T value : property.func_177700_c()) {
            builder.partialState().with((Property)property, (Comparable)value).addModels(new ConfiguredModel[] { new ConfiguredModel(this.model(NameUtil.suffixPath(key, "_" + value.toString()))) });
        }
    }
    ...
    private <T extends Comparable<T>> void allStateSuffixMultiLayerModel(final Block b) {
        ...
        final VariantBlockStateBuilder builder = this.getVariantBuilder(b);
        for (final T value : property.func_177700_c()) {
            builder.partialState().with((Property)property, (Comparable)value).addModels(new ConfiguredModel[] { new ConfiguredModel(this.multiLayerModel(NameUtil.suffixPath(key, "_" + value.toString()))) });
        }
    }
