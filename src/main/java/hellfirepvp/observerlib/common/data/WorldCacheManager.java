...
public class WorldCacheManager implements ITickHandler {
    ...
    public static WorldCacheManager getInstance() { return instance; }
        ...
        for (WorldCacheDomain domain : domains.values()) {
            for (ResourceLocation dimTypeName : domain.getUsedWorlds()) {
                for (WorldCacheDomain.SaveKey<?> key : domain.getKnownSaveKeys()) {
                    ...
                    if (data != null && data.needsSaving()) WorldCacheIOThread.scheduleSave(domain, dimTypeName, data);
                }
            }
        }
    }
    ...
    public static void cleanUp() {
        for (WorldCacheDomain domain : domains.values()) domain.clear();
    }

    @Nonnull
    public static WorldCacheDomain createDomain(String modid) {
        ResourceLocation domainKey = new ResourceLocation(modid, "worlddata");
    ...
    public static WorldCacheDomain findDomain(String modid) {
        ResourceLocation domainKey = new ResourceLocation(modid, "worlddata");
        return domains.getOrDefault(domainKey, null);
    }
    ...
    public void tick(TickEvent.Type type, Object... context) {
        ...
        for (WorldCacheDomain domain : domains.values()) domain.tick(world);
    }
    ...
    public void doSave(Level world) {
        ...
        for (WorldCacheDomain domain : domains.values()) {
            for (WorldCacheDomain.SaveKey<?> key : domain.getKnownSaveKeys()) {
                ...
                if (data != null && data.needsSaving()) WorldCacheIOThread.scheduleSave(domain, worldName, data);
            }
        }
    }
    ...
    @Override
    public String getName() { return "WorldCacheManager"; }
}
