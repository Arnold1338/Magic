...
public class MatcherObserverHelper extends ObserverHelper {
    ...
    public <T extends ChangeObserver> ChangeSubscriber<T> observeArea(Level world, BlockPos center, ObserverProvider provider) {
        return (ChangeSubscriber<T>) getBuffer(world).observeArea(world, center, provider);
    }

    @Override
    public boolean removeObserver(Level world, BlockPos pos) { return getBuffer(world).removeSubscriber(pos); }
